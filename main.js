...
function start_bot() {
    ...
    bot.on("spawn", () => {
        ...
        sendDiscordWebhook(
            `🟢 **${process.env}** đã kết nối và đăng nhập thành công vào server!`,
        );
    ...
    bot.on("entityHurt", (entity) => {
        if (entity === bot.entity) {
            const uptime = getUptimeString();
            const msg = `⚠️ **[CẢNH BÁO]** **${process.env}** đang bị tấn công hoặc nhận sát thương! (Đã treo được: \`${uptime}\`)`;
    ...
    bot.on("death", () => {
        ...
        console.log("im dead");
        sendDiscordWebhook(
            `💀 **${process.env}** đã bị tiêu diệt! (Thời gian đã sống: \`${uptime}\`) -> Đang chờ hồi sinh...`,
        );
    ...
    bot.on("kicked", (reason) => {
        ...
        const msg = `🔴 **[BỊ KICK]** **${process.env}** đã bị đuổi khỏi server!.
    💬 Lý do: \`${reasonClean}\`
    ...
    botInstance.on("end", () => {
        ...
        if (wasKicked) {
            ...
            sendDiscordWebhook(
                `⚠️ **${process.env}** bị mất kết nối! (Thời gian đã treo trước đó: \`${uptime}\`). Đang tiến hành kết nối lại sau 10 giây...`,
            );
        }
        startTime = null;
...
function exitBot() {
    ...
    const uptime = getUptimeString();
    sendDiscordWebhook(
        `🔴 **${process.env}** đã chủ động thoát game. (Tổng thời gian đã treo: \`${uptime}\`)`,
    );
    startTime = null;
