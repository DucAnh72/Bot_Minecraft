...
function sendDiscordWebhook(content) {
    ...
    const req = https.request(options, (res) => {
        if (res.statusCode >= 400) {
            console.error(`[!] Discord Webhook từ chối gửi. Mã lỗi: ${res.statusCode}`);
        }
    });
...
function start_bot() {
    ...
    bot.on('spawn', () => {
        ...
        sendDiscordWebhook(`🟢 **${config.username}** đã kết nối và đăng nhập thành công vào server!`);
    ...
    bot.on('death', () => {
        const uptime = getUptimeString();
        console.log('im dead')
        sendDiscordWebhook(`💀 **${config.username}** đã bị tiêu diệt! (Thời gian đã sống: \`${uptime}\`) -> Đang chờ hồi sinh...`);
    ...
    const msg =
    `🔴 **[BỊ KICK]** **${config.username}** đã bị đuổi khỏi server!.
    💬 Lý do: \`${reasonClean}\`
    ...
    bot.on('messagestr', (message, messagePosition) => {
        ...
        const channel =
        discordClient.channels.cache.get(CHANNEL_ID);
        ...
        if (msgLower.includes('banned') || msgLower.includes('bị khóa tài khoản') || msgLower.includes('ban lệnh')) {
            const uptime = getUptimeString();
            sendDiscordWebhook(`🚫 **[CẢNH BÁO BAN]** Phát hiện tin nhắn nghi ngờ Bot bị BAN:\n\`${message}\`\n⏱️ Thời gian đã treo: \`${uptime}\``);
        }
    })
    ...
    bot.on('end', () => {
        ...
        if (wasKicked) {
            wasKicked = false;
            console.log('Bot đã bị kick khỏi server. Đang tiến hành kết nối lại sau 10 giây...')
        }
        else {
        const uptime = getUptimeString();
        console.log('Disconnected')
        console.log('[+] Kết Nối Lại Sau 10s')
        sendDiscordWebhook(`⚠️ **${config.username}** bị mất kết nối! (Thời gian đã treo trước đó: \`${uptime}\`). Đang tiến hành kết nối lại sau 10 giây...`);
        }
        startTime = null; 
...
function startOrder() {
    ...
    sendDiscordWebhook(`🔄 **[AUTO ORDER]** Đã bật auto order (Thời gian đã treo: \`${getUptimeString()}\`)`);
...
function stopOrder() {
    ...
    sendDiscordWebhook(`🔄 **[AUTO ORDER]** Đã tắt auto order (Thời gian đã treo: \`${getUptimeString()}\`)`);
}
